...
public class AiConfig {
        ...
        if (configs != null && !configs.isEmpty()) {
            for (LlmConfig config : configs) {
                if (config.getName() == null || config.getName().trim().isEmpty()) {
                    throw new IllegalStateException("大模型配置中 name 不能为空");
                }

                if (config.getApiKey() == null || config.getApiKey().trim().isEmpty()) {
                    throw new IllegalStateException("大模型配置中 api-key 不能为空：" + config.getName());
                }

                if (config.getBaseUrl() == null || config.getBaseUrl().trim().isEmpty()) {
                    throw new IllegalStateException("大模型配置中 base-url 不能为空：" + config.getName());
                }

                if (config.getModel() == null || config.getModel().trim().isEmpty()) {
                    throw new IllegalStateException("大模型配置中 model 不能为空：" + config.getName());
                }

                // 创建 OpenAI API 客户端（兼容 DeepSeek 等 OpenAI 兼容的 API）
    ...
    /**
     * 默认 ChatClient（向后兼容，使用第一个配置的模型）
     * 如果没有配置的模型，返回 null（测试环境可能没有配置）
     */
    @Bean
    @Primary
    public ChatClient chatClient(Map<String, ChatModel> chatModelMap) {
        if (chatModelMap == null || chatModelMap.isEmpty()) {
            System.out.println("[AiConfig] 未配置 AI 模型，跳过 ChatClient 创建（测试环境）");
            return null;
        }
    ...
    public static class LlmConfigProperties {
        ...
        public List<LlmConfig> getConfigs() {
            return configs;
        }
